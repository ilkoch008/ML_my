...
class Sequential(Module):
    ...
    def updateOutput(self, input):
        ...
        for i in range(1, self.modules.__len__()):
    ...
    def backward(self, input, gradOutput):
        ...
        """
        # Your code goes here. ################################################
        n = self.modules.__len__()
        g = self.modules[n-1].backward(self.modules[n-2].output,gradOutput)
...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        local_derivative = self.output[:,:,None] * (local_derivative - self.output[:,None,:])
...
class BatchNormalization(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        # ivar = 1/sqrtvar
        # x_ = xu * ivar
        # dxu1 = gradOutput * ivar
        # divar = np.sum(gradOutput * xu, axis=0)
        # dsqrtvar = divar * (-1) / (sqrtvar ** 2)
        # dvar = 0.5 / np.sqrt(var + self.EPS) * dsqrtvar
        # dsq = 1/N * np.ones((N,D)) * dvar
        # dxu2 = 2 * xu * dsq
        # dx1 = dxu1 + dxu2
        # du = -1.0 * np.sum(dx1, axis=0)
        # dx2 = 1/N * np.ones((N,D)) * du
        # self.gradInput = dx1 + dx2

        var_sqrt = np.sqrt(np.var(input, axis=0) + self.EPS)
        sums = np.mean(gradOutput * self.output, axis=0)
        self.gradInput = (gradOutput - gradOutput.mean(axis=0) - self.output * sums)/var_sqrt
        return self.gradInput
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training :
            N = input.shape[0]
            D = input.shape[1]
            self.mask = (np.random.rand(N, D) < (1 - self.p)) * 1.0 / (1.0 - self.p)
...
class LeakyReLU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        a = np.multiply(gradOutput , input > 0)
        self.gradInput = np.multiply(gradOutput * self.slope , input < 0)
        self.gradInput += a
        return self.gradInput
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = np.multiply(self.alpha * np.exp(input) - 1, input < 0)
        a = np.multiply(input, input > 0)
        self.output += a
        return  self.output
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        a = np.multiply(gradOutput , input > 0)
        self.gradInput = np.multiply(gradOutput * self.alpha * np.exp(input) , input < 0)
        self.gradInput += a
        return self.gradInput
