...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    yy = torch.ones((x_1.shape[0], x_2.shape[0], x_1.shape[1]), dtype=torch.float32) * x_2
    xx = torch.ones((x_2.shape[0], x_1.shape[0], x_1.shape[1]), dtype=torch.float32) * x_1
    xx = torch.transpose(xx, 1, 0)
    distances = torch.exp(- torch.sum((xx - yy)**2, 2) / (2 * sigma**2)) ### YOUR CODE HERE
...
def hinge_loss(scores, labels):
    ...
    loss = 1 - torch.mul(scores, labels)
    for i, l in enumerate(loss):
        if l < 0:
            loss[i] = 0
    return torch.mean(loss)  ### YOUR CODE HERE
