...
def mad_median(y):
    ...
    return np.sum(np.absolute(y - np.median(y))) / y.shape[0]
...
class DecisionTree(BaseEstimator):
    ...
    def choose_best_split(self, X_subset, y_subset):
        ...
        threshold = .0
        inf_gain = .0
        best_inf_gain = -100000
        n_features = X_subset.shape[1]
        for i in range(n_features):
            for j in np.unique(X_subset[:, i]):
                left, right = self.make_split_only_y(i, j, X_subset, y_subset)
                if (left.shape[0] == 0) or (right.shape[0] == 0):
                    continue
                inf_gain = - left.shape[0] / p_y * criterion(left) - right.shape[0] / p_y * criterion(right)
    ...
    def predict_proba(self, X):
        ...
        for i, obj in enumerate(X):
            node = self.root
            while node.left_child is not None or node.right_child is not None:
                if obj[node.feature_index] < node.value and node.left_child is not None:
                    node = node.left_child
                elif node.right_child is not None:
                    node = node.right_child
